...
public class FhirMapperService : IFhirMapperService
{
    ...
    public Task<string> GenerateOPConsultationBundleAsync(string fhirJsonPayload)
    {
        // Parse the simple input JSON
        using var document = JsonDocument.Parse(fhirJsonPayload);
        var root = document.RootElement;
        ...
        var opConsultSection = new Composition.SectionComponent
        {
            ...
            Code = new CodeableConcept("http://snomed.info/sct", "371530004", "Clinical consultation report")
        };
        ...
        // Add observation for notes
        var notes = "OP Consultation notes";
        if (root.TryGetProperty("clinicalNotes", out var notesProp))
        {
        ...
        var observation = new Observation
        {
            ...
            Code = new CodeableConcept("http://snomed.info/sct", "11429006", "Consultation"),
            Subject = new ResourceReference($"Patient/{patient.Id}"),
        ...
        // Add medication requests if prescriptions exist
        if (root.TryGetProperty("prescriptions", out var prescriptionsElement))
        {
            int medIndex = 1;
            foreach (var p in prescriptionsElement.EnumerateArray())
            {
                ...
                {
                    Id = $"Medication-{medIndex}",
                    Code = new CodeableConcept("http://snomed.info/sct", "0000", medName)
                };
    ...
    public Task<string> GenerateHealthDocumentBundleAsync(string fhirJsonPayload)
    {
        // Parse the simple input JSON
        using var document = JsonDocument.Parse(fhirJsonPayload);
        var root = document.RootElement;
        ...
        var composition = new Composition
        {
            ...
            Identifier = new Identifier { System = "https://ndhm.in", Value = Guid.NewGuid().ToString() },
            Status = CompositionStatus.Final,
            Type = new CodeableConcept("http://snomed.info/sct", "419891008", "Record artifact"),
            Subject = new ResourceReference($"Patient/{patient.Id}"),
            Encounter = new ResourceReference($"Encounter/{encounter.Id}"),
            DateElement = new FhirDateTime(authoredOn),
            Author = new List<ResourceReference> { new ResourceReference($"Practitioner/{practitioner.Id}") },
        ...
        {
            Title = "Record Artifact",
            Code = new CodeableConcept("http://snomed.info/sct", "419891008", "Record artifact")
        };
        ...
        if (root.TryGetProperty("documents", out var documentsElement))
        {
            ...
            foreach (var d in documentsElement.EnumerateArray())
            {
                ...
                if (!string.IsNullOrEmpty(dataBase64))
                {
                    ...
                    {
                        // Fallback or ignore malformed base64 for safety
                    }
                }
