...
public class AbdmM1Controller : ControllerBase
{
    ...
    private static readonly ConcurrentDictionary<string, (string Code, DateTime Expiry)> _captchaStore 
        = new ConcurrentDictionary<string, (string Code, DateTime Expiry)>();
    ...
    public IActionResult GetCaptcha()
    {
        ...
        var txnId = "CPT_" + Guid.NewGuid().ToString("N").Substring(0, 10);

        _captchaStore[txnId] = (code, DateTime.UtcNow.AddMinutes(10));

        _logger.LogInformation($"M1 Captcha Generated: TxnId={txnId}, Code={code}");
    ...
    public async Task<IActionResult> VerifyOtp([FromBody] AbdmVerifyOtpRequest request)
    {
        ...
        if (response.Success && response.Data != null)
        {
            ...
            // Extract xToken from profile data and emit as response header + body field
            string? xToken = response.Data?.Token;
            if (!string.IsNullOrEmpty(xToken))
            {
                // Normalize: strip 'Bearer ' prefix if present
                if (xToken.StartsWith("Bearer ", StringComparison.OrdinalIgnoreCase))
                    xToken = xToken.Substring(7).Trim();
                Response.Headers["X-Token"] = xToken;
                _logger.LogInformation($"M1 VerifyOtp: Emitting X-Token in header (first 20): {xToken.Substring(0, Math.Min(20, xToken.Length))}...");
            }
            else
            {
                _logger.LogWarning("M1 VerifyOtp: xToken/Token is null in profile data. Check AbhaProfile model fields.");
            }
    ...
    public async Task<IActionResult> LoginVerify([FromBody] AbdmVerifyOtpRequest request)
    {
        ...
        if (response.Success && response.Data != null)
        {
            ...
            // Emit X-Token in response header
            string? xToken = response.Data?.Token;
            if (!string.IsNullOrEmpty(xToken))
            {
                if (xToken.StartsWith("Bearer ", StringComparison.OrdinalIgnoreCase))
                    xToken = xToken.Substring(7).Trim();
                Response.Headers["X-Token"] = xToken;
                _logger.LogInformation($"M1 LoginVerify: Emitting X-Token in header.");
            }
