...
class ScanShareView extends GetView<ScanShareController> {
  ...
  Widget _buildHospitalQrCard(BuildContext context, ScanShareController controller) {
    return Container(
      padding: const EdgeInsets.all(24),
      decoration: glassDecoration(),
      child: Column(
        ...
        children: [
          Row(
            ...
            children: [
              Row(
                children: [
                  const Icon(Icons.storefront_outlined, color: AppColor.accent, size: 22),
            ...
            children: [
              Expanded(
                ...
                child: Column(
                  crossAxisAlignment: CrossAxisAlignment.start,
                  children: [
                    Text("HIP ID (Registered ABDM ID):", style: fontMedium.copyWith(fontSize: 11)),
                    const SizedBox(height: 4),
                    TextField(
              ...
              Expanded(
                ...
                child: Column(
                  crossAxisAlignment: CrossAxisAlignment.start,
                  children: [
                    Text("Select OPD Counter Desk:", style: fontMedium.copyWith(fontSize: 11)),
                    const SizedBox(height: 4),
                    Obx(() => Container(
                          ...
                          child: DropdownButtonHideUnderline(
                            child: DropdownButton<String>(
                              ...
                              isExpanded: true,
                              isDense: true,
                              items: controller.counterNames.entries.map((entry) {
                                return DropdownMenuItem<String>(
                                  value: entry.key,
                                  child: Text(entry.value, style: fontMedium.copyWith(fontSize: 12)),
                                );
                              }).toList(),
                              onChanged: (val) {
                                if (val != null) controller.selectedCounterId.value = val;
                              },
                            ),
                          ),
                        )),
          ...
          Text("Select ABDM QR Format Mode:", style: fontMedium.copyWith(fontSize: 11)),
          const SizedBox(height: 6),
          Obx(() => Row(
                children: [
                  Expanded(
                    child: ChoiceChip(
                      label: Text("ABDM Universal JSON", style: fontSmall.copyWith(fontSize: 11)),
                  ...
                  Expanded(
                    child: ChoiceChip(
                      label: Text("Official DeepLink URL", style: fontSmall.copyWith(fontSize: 11)),
                      selected: controller.qrFormatMode.value == 'deeplink',
                      selectedColor: AppColor.success.withOpacity(0.2),
                      onSelected: (selected) {
                        if (selected) controller.qrFormatMode.value = 'deeplink';
                      },
                    ),
                  ),
                ],
            ...
            child: Obx(() {
              ...
              return Container(
                ...
                decoration: BoxDecoration(
                  ...
                  border: Border.all(color: AppColor.accent.withOpacity(0.3), width: 2),
                  boxShadow: [
                    BoxShadow(
                  ...
                  children: [
                    ...
                    Container(
                      ...
                      child: Text(
                        ...
                        style: fontBold.copyWith(fontSize: 12, color: AppColor.accent, letterSpacing: 1.1),
                      ),
                    ),
                  ],
                ),
                  ...
                  children: [
                    Expanded(
                      child: Text(
                        ...
                        style: fontRegular.copyWith(fontSize: 11, color: AppColor.textSecondary, fontFamily: 'monospace'),
                        maxLines: 2,
  ...
  Widget _buildLiveTokensTable(BuildContext context, ScanShareController controller) {
    return Container(
      padding: const EdgeInsets.all(24),
      decoration: glassDecoration(),
      child: Column(
        ...
        children: [
          Row(
            ...
            children: [
              Row(
                children: [
                  const Icon(Icons.confirmation_number_outlined, color: AppColor.accent, size: 22),
                  const SizedBox(width: 8),
                  Text(
                  ...
                  Obx(() => OutlinedButton.icon(
                        ...
                        icon: controller.isFetchingRemote.value
                            ? const SizedBox(
                                width: 12,
                                height: 12,
                                child: CircularProgressIndicator(strokeWidth: 2, color: AppColor.success),
                              )
                            : const Icon(Icons.sync, size: 14, color: AppColor.success),
                        label: Text(
                          controller.isFetchingRemote.value ? "Syncing..." : "Sync Real Scans",
                          style: fontSmall.copyWith(color: AppColor.success, fontWeight: FontWeight.bold, fontSize: 11),
                        ),
                      )),
                  const SizedBox(width: 8),
                  Obx(() => Container(
          ...
          Obx(() {
            ...
            return ListView.separated(
              ...
              separatorBuilder: (context, index) => Divider(color: AppColor.border.withOpacity(0.5), height: 1),
                              ...
                              Row(
                                children: [
                                  ...
                                  Container(
                                    ...
                                    child: Text(
                                      token.gender,
                                      style: fontSmall.copyWith(fontSize: 10, fontWeight: FontWeight.bold),
                                    ),
                                  ),
                                ],
                              ),
                      ...
                      Row(
                        children: [
                          ...
                          ElevatedButton.icon(
                            ...
                            onPressed: () => _showLinkContextModal(context, token),
                            icon: const Icon(Icons.add_link, color: Colors.white, size: 14),
                            label: Text(
  ...
  void _showLinkContextModal(BuildContext context, OpdTokenRecord token) {
    final visitRefController = TextEditingController(text: 'OPD-VISIT-${DateTime.now().millisecondsSinceEpoch % 10000}');
            ...
            children: [
                ...
                children: [
                  Row(
                    children: [
                      ...
                      const SizedBox(width: 10),
                      Text("Link Care Context (Milestone M2)", style: fontBold.copyWith(fontSize: 16)),
                    ],
                  ),
                  IconButton(
              ...
              Container(
                ...
                child: Column(
                  crossAxisAlignment: CrossAxisAlignment.start,
                  children: [
                    Text("Token #${token.tokenNumber} • ${token.patientName}", style: fontBold.copyWith(fontSize: 14)),
                    const SizedBox(height: 4),
                    Text("ABHA Address: ${token.abhaAddress}", style: fontSmall.copyWith(color: AppColor.accent, fontWeight: FontWeight.bold)),
                  ],
                ),
              ),
              const SizedBox(height: 16),
              ...
              Text("Enter OPD Visit / Medical Record Reference:", style: fontMedium.copyWith(fontSize: 12)),
              const SizedBox(height: 6),
              TextField(
              ...
              Row(
                ...
                children: [
                  OutlinedButton(
                    onPressed: () => Navigator.of(context).pop(),
                    child: Text("Cancel", style: fontMedium.copyWith(fontSize: 12)),
                  ),
                  ...
                  ElevatedButton.icon(
                    ...
                    onPressed: () async {
                      ...
                      await careContextController.linkNewCareContext(token.abhaAddress, visitRefController.text.trim());
                      Get.toNamed(Routes.M2_DISCOVERY);
                    },
                    icon: const Icon(Icons.check_circle_outline, color: Colors.white, size: 16),
                    label: Text("Link Context & View Records", style: fontBold.copyWith(color: Colors.white, fontSize: 12)),
                  ),
                ],
              ),
            ],
